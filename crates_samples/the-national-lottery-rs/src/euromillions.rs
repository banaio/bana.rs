...
pub const MAIN_MAX: u64 = 50;
pub const EXTRA_TOTAL: usize = 2;
pub const EXTRA_MAX: u64 = 12;
...
impl std::fmt::Display for Play {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        ...
        // let comma_separated = self.numbers.into_iter().map(|i| i.to_string()).collect::<Vec<_>>().join(", ");
...
pub fn run(no_of_plays: u128) -> Result<(), std::io::Error> {
    ...
    let color = termion::color::Fg(termion::color::Yellow);
    ...
    println!("{}euromillions{}", color, termion::style::Reset);
    println!("{}", plays);
    println!("{}{}{}", color, separator, termion::style::Reset);
